            ...
            <div class="collapse navbar-collapse mb-2" id="navbarSupportedContent">
                ...
                <li class="nav-item ms-4">
                    <form class="d-flex mt-2" role="search" action="/home">
                        @csrf
                        <input class="form-control me-2" type="search" placeholder="Search Film" aria-label="Search" name="cari" value="{{ request('cari') }}">
                ...
                <a href="/profile" class="prof">
                    <div class=" d-flex mt-1">
                        <h4 class="profile mt-4 fw-semibold" data-bs-toggle="dropdown" aria-expanded="false">{{ $user->username }}</h4>
                        @if ($user->gambar == null)
                        <img src="/assets/img/profile.png" class="gambarprof ms-3 me-4 mt-1 rounded-circle">
                        @else
                        <img src="{{ Storage::url('public/users/') . $user->gambar }}" class="gambarprof ms-3 me-4 mt-1 rounded-circle">
                        @endif
                    </div>
                </a>
            </div>
